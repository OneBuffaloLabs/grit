    ...
    switch (challenge.status) {
      case 'active':
        return {
          ...
          text: `Day ${completedDays + 1} of ${totalDays}`,
        };
      case 'completed':
        return {
          icon: <FontAwesomeIcon icon={faCheck} className="text-green-500" title="Completed" />,
          text: `Completed ${completedDays} of ${totalDays} days`,
        };
      case 'failed':
        return {
...
const NewChallengeModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleCreateChallenge = async (type: '75 Hard') => {
    setIsCreating(true);
    try {
      const newChallenge = await startNewChallenge(type);
      if (newChallenge) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
      <div
        className="bg-[var(--color-surface)] p-6 rounded-lg max-w-md w-full"
          ...
          <button
            ...
            <div>
              ...
              <p className="text-sm text-[var(--color-text-muted)]">
                The original mental toughness challenge.
              </p>
            </div>
